...
impl V8JsRuleEngine {
    ...
    fn create_and_execute(&self, method: &str, url: &str, scheme: &str, host: &str, path: &str) 
...
mod tests {
    ...
    async fn test_js_rule_allow() {
        ...
        let result = engine.evaluate(Method::GET, "https://github.com/test").await;
    ...
    async fn test_js_rule_deny() {
        ...
        let result = engine.evaluate(Method::GET, "https://example.com/test").await;
    ...
    async fn test_js_rule_with_method() {
        ...
        let result = engine.evaluate(Method::GET, "https://api.github.com/v3").await;
        assert!(matches!(result.action, Action::Allow));
        
        let result = engine.evaluate(Method::POST, "https://api.github.com/v3").await;
        assert!(matches!(result.action, Action::Deny));
    }
    ...
    async fn test_js_rule_with_path() {
        ...
        let result = engine.evaluate(Method::GET, "https://example.com/api/test").await;
        assert!(matches!(result.action, Action::Allow));
        
        let result = engine.evaluate(Method::GET, "https://example.com/public/test").await;
        assert!(matches!(result.action, Action::Deny));
    }
    ...
    async fn test_js_rule_complex_logic() {
        ...
        // Test GitHub allow
        let result = engine.evaluate(Method::GET, "https://github.com/user/repo").await;
        assert!(matches!(result.action, Action::Allow));
        
        // Test social media block
        let result = engine.evaluate(Method::GET, "https://facebook.com/profile").await;
        assert!(matches!(result.action, Action::Deny));
        
        // Test API allow
        let result = engine.evaluate(Method::POST, "https://example.com/api/data").await;
        assert!(matches!(result.action, Action::Allow));
        
        // Test default deny
        let result = engine.evaluate(Method::GET, "https://example.com/public").await;
        assert!(matches!(result.action, Action::Deny));
    }
