...
impl KeychainManager {
    ...
    pub fn install_ca(&self, cert_path: &Path) -> Result<()> {
        // Check if there are existing certificates and warn
        let find_output = Command::new("security")
            .args(["find-certificate", "-c", CA_NAME])
            .arg(self.get_user_keychain()?)
            .output()
            .context("Failed to check for existing certificates")?;

        if find_output.status.success() {
            println!(
                "⚠️  Warning: Existing {} certificate(s) found in keychain",
                CA_NAME
            );
            println!("   These will be removed and replaced with the new certificate.");
        }
